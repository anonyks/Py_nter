...
class MagnifierTool(Tool):
    # hover over canvas to see zoomed view. scroll to adjust zoom. doesnt change canvas
    ...
    def preview(self, screen):
        ...
        # subsurface grabs a chunk of canvas without copying all of it
        # copy() so we get a separate image we can scale
        snippet = g.canvas_surface.subsurface(src_rect).copy()

        # scale up the tiny snippet to zoom size
